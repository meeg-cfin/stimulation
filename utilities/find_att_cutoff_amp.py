...
the input voltage limit of the CFIN audio attenuator. This script allows for a manual
routine to find the current limit. *This test only needs to be performed once, after
any change to the sound-generating hardware.*

Method
------

Run the script, which sends simple 100Hz sounds (100 ms) at 2Hz. Control the sound card
output volume using these keys:
...
from psychopy import sound, core, visual, parallel, event
sound.init(rate=44100, stereo=True, buffer=128)
