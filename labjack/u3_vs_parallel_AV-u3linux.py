...
try:
    ...
    # This doesn't work after all: the call to getFeedback seems to be blocking :(
    # Will have to run as a separate thread? But then still same issue, perhaps...
...
#                   u3.WaitShort(Time=int(duration/128.e-6)),
#                   u3.WaitLong(Time=int(duration/32.e-3)),
...
bckColour = '#000000'
monitor = 'testMonitor'
frameRate = 60.0  # on linux & pyglet, is detected as 30 Hz :(
frameRate = 30.0  # on linux & pyglet, is detected as 30 Hz :(
stimSize = 8.
stimCycPerDeg = 1.
angCycles = 0.
...
#    rad_stim.draw()
    ...
    #trigger_U3(code=curCode, duration=0.9)
